        ...
        <div class="modal-content">          
          ...
          <div class="modal-body">
            <div>
              ...
              <!-- <input type="hidden" name="id_unik" id="id_unik" class="form-control"  /> -->
              <input type="text" name="tangal_ujian" id="tangal_ujian" class="form-control" required />
            </div>
          </div>
          <!-- Modal footer -->
  ...
  <script type="text/javascript">
    $(document).ready( function () {
      ...
      var table= $('#santritable').DataTable({
        ...
        order: [],
        ajax: {
          url: "<?php echo site_url('santri/data_list_jadwal')?>",
          type: "POST"
        },
        "columnDefs": [{
      ...
      // $('#santritable tbody').on( 'click', 'tr', function () {
      //   $(this).toggleClass('selected');
      //   var datacol = table.rows('.selected').data()[0];
      //   console.log(datacol[4]);
      // } );
      //  $('#santritable tbody').on( 'click', 'button', function () {
      //   table.row( $(this).toggleClass('selected'));
      //   var data = table.row( $(this).parents('tr') ).data();
      //   var email = data[ 4 ];
      //   emailsatri.push(email);
    ...
    function updateUser(idu) {
      ...
      modal.modal();
            $.ajax({
              type: "POST",
              url: '<?php  echo base_url(); ?>santri/data_detail_jadwal',
              data:{
                idu:idu
              },
              dataType:'json',
              success: function(data) {
                $.each(data.hasildata,function(i,v){
                  $('#id_unik').val(v.iduniq);
                  $('#tangal_ujian').val(v.tanggal);
                });
              }
            });
