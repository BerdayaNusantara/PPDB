    ...
    <div class="content-wrapper">
      ...
      <div class="content-header">
        <div class="container-fluid">
          <div class="row mb-2">
            <div class="col-sm-6">
              <h1 class="m-0 text-dark">Jadwal Pendaftaran</h1>
            </div><!-- /.col -->
            <div class="col-sm-6">
        ...
        <div class="modal-content">          
          ...
          <div class="modal-body">
            <div>
              ...
              <input name="tanggal_mulai" id="tanggal_mulai" class="form-control" require/>
            </div>
            <div>
              <label>Tanggal Selesai</label>
              <input name="tanggal_selesai" id="tanggal_selesai" class="form-control" require/>
            </div>
            <div>
              <label>Nama</label>
...
<script type="text/javascript">
    $(document).ready( function () {
        $('#usertable').DataTable({
            ...
            order: [],
            ajax: {
                url: "<?php echo site_url('konten/data_jadwal')?>",
                type: "POST"
            },
            "columnDefs": [{
    ...
    function tambahData() {
        $('#id').val("");
        var modal = $("#modalUser");
        modal.find('.modal-title').text('Tambah Data Jadwal');
        $("#modalUser").modal();
    };
    ...
    $(function () {
        $('#formUser').on('submit', function (e) {
            e.preventDefault();
            $.ajax({
                url: "<?php  echo base_url(); ?>konten/jadwal_simpan",
                type: "POST",
    ...
    function hapusData() {
        ...
        $.ajax({
            url: "<?php  echo base_url(); ?>konten/jadwal_hapus",
            type: "POST",
            data:  {id,id},
            dataType:'json', 
            success: function(data)
    ...
    function updateJadwal(id) {
        $('#id').val(id);
        var modal = $("#modalUser");
        modal.find('.modal-title').text('Edit Data Jadwal');
        modal.modal();
        $.ajax({
            type: "POST",
            url: '<?php  echo base_url(); ?>konten/jadwal_detail',
            data:{
                id:id
            },
            dataType:'json',
            success: function(data) {
                $.each(data.hasildata,function(i,v){
                    $('#id').val(v.id);
                    $('#tanggal_mulai').val(v.tanggal_mulai_format);
                    $('#tanggal_selesai').val(v.tanggal_selesai_format);
                    $('#nama').val(v.nama);
                });
            }
        });
