...
class santri extends CI_Controller {
	...
	public function simpan_profile()
	{
		...
		if(($logged=="yes")&&($level=="Santri"))
		{
			...
			$datainsert["tinggi_badan"]=$tinggi_badan;
			$datainsert["berat_badan"]=$berat_badan;
			$datainsert["lingkar_kepala"]=$lingkar_kepala;
			$datainsert["anak_ke"]=$anak_ke;
			$datainsert["jml_saudara"]=$jml_saudara;
			$datainsert["status_anak"]=$status_anak;
			
			$datainsert["riwayat_penyakit"]=$riwayat_penyakit;
			$datainsert["nama_saudara"]=$nama_saudara;
	...
	public function cetak_kartu()
	{
		...
		if(($logged=="yes")&&($level=="Santri"))
		{
			...
			$doc_ktp_ibu = $this->m_siswa->listing_dokumen(" WHERE iduniq='$id_santri' AND type='KTP Ibu' ");
			$doc_ktp_bapak = $this->m_siswa->listing_dokumen(" WHERE iduniq='$id_santri' AND type='KTP Ayah' ");
			...
			if(count($doc_ktp_ibu)>0)
			{$stat_ktp_ibu="1";}else{$stat_ktp_ibu="0";}
			if(count($doc_ktp_bapak)>0)
			{$stat_ktp_bapak="1";}else{$stat_ktp_bapak="0";}
			
			if(($stat_kk=="1")&&($stat_akta=="1")&&($stat_ktp_ibu=="1")&&($stat_ktp_bapak=="1"))
			{
			    $dokumen_status = "1";
			}else{
			    $dokumen_status = "0";
		...
		if(($logged=="yes")&&($level=="Administrator")||($level=="UnitTKIT")||($level=="UnitSDIT")||($level=="UnitSMPIT")||($level=="UnitSMAIT"))
		{
		    ...
		    foreach ($list as $l) {
		      ...
		      $row[] = "<div style='text-align: center;'>
		      				<a href=".base_url()."backoffice/santri_detail/".$l->id_uniq." class='btn btn-success btn-sm'><span class='fa fa-eye'></span></a>
			...
			if(count($data_santri)>0)
			{
			    foreach($data_santri as $ds){
			        ...
			        if($jenjang=="TKIT A"){
			            ...
			            $jenjang="SMPIT Akademik";
			        }else if($jenjang=="SMAIT"){
			            $jenjang="SMAIT";
			        }
			    }
			}
			$data_jadwal = $this->m_jadwalujian->listingview2(" WHERE jenjang='$jenjang'");
	...
	public function jadwal_data()
	{
		...
		if(($logged=="yes")&&($level=="Administrator")||($level=="UnitTKIT")||($level=="UnitSDIT")||($level=="UnitSMPIT")||($level=="UnitSMAIT"))
		{
		    ...
		    foreach ($list as $l) {
		      ...
		      $row[] = "<div style='text-align: center;'>
		      				<a href='".base_url()."santri/jadwal_peserta/".$l->id."' class='btn btn-sm btn-info'><span class='fa fa-eye'></span> Lihat Peserta</a>
	...
	public function dokumen_ktp_ibu()
	{
	    $logged = $this->session->userdata("logged_in");
		$level = $this->session->userdata("level");
		if(($logged=="yes")&&($level=="Santri")||($level=="Administrator"))
		{
			...
			if(!empty($_FILES['ktp_ibu']['name'])){
				$config['file_name']= "ktp_ibu_".$nisn;
				$config['upload_path']='./upload/santri/'.$nisn;
				$config['allowed_types']='jpg|png|jpeg|pdf|JPG|PNG|JPEG|PDF';
				$config['max_size']='50000';
				$config['max_width']='0';
				...
				if (!$this->upload->do_upload('ktp_ibu'))
				{
					...
					$data = array('upload_data' => $this->upload->data());
					$eks = explode('.', $_FILES['ktp_ibu']['name']);
					$eks = $eks[count($eks)-1];
	...
	public function detail_ktp_ibu()
	{
		$logged = $this->session->userdata("logged_in");
		$level = $this->session->userdata("level");
		if(($logged=="yes")&&($level=="Santri")||($level=="Administrator"))
		{
			$id=$this->input->post('id');
			$data_calon = $this->m_siswa->listing_dokumen(" WHERE iduniq='$id' AND type='KTP Ibu' ");
		    $X = array('hasildata'=>$data_calon);               
		    echo json_encode($X);
		}else{
			redirect("home/login");
